...
void	check_cone(t_obj *cone)
{
	if (cone->radius < 0.1 ||\
		...
		cone->rot.x < -360 || cone->rot.x > 360 || \
		cone->rot.y < -360 || cone->rot.y > 360 ||\
		cone->rot.z < -360 || cone->rot.z > 360)
		error_cone();
}
...
void	check_cylinder(t_obj *cyl)
{
	if (cyl->color.x < 0 || cyl->color.x > 255\
		...
		cyl->radius < 0.1 ||\
		cyl->rot.x < -360 || cyl->rot.x > 360 || \
		cyl->rot.y < -360 || cyl->rot.y > 360 ||\
		cyl->rot.z < -360 || cyl->rot.z > 360)
		error_cyl();
}
